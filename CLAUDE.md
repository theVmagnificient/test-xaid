...
- **Blog articles** are generated by the site-poster pipeline and integrated here via the 4-file contract (see `../site-poster/CLAUDE.md`): a metadata entry in `src/data/blog-posts.ts`, a page component in `src/pages/blog/`, a route in `src/App.tsx`, and a slug in `package.json`'s `reactSnap.include`.
