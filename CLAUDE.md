...
- Reusable article components: `src/components/BlogCTA.tsx` (end-of-article CTA) and `src/components/KeyStats.tsx` (the 4-up "key stats" figure band — `<KeyStats items={[{ stat, label, sub }, …]} />`). **New articles must use `KeyStats`, never hand-rolled grid markup.** The 40 articles written before 2026-08-11 still inline that markup; that duplication is exactly why a one-word overflow needed a 40-file sed and stalled the pipeline. Leave them, but don't add a 41st.
...
- **HARD TOPIC RULE — no competitor airtime (founder 2026-08-11):** never build a blog article around a competitor's or competitor-adjacent vendor's partnership, alliance, funding round or product launch (Viz.ai, Aidoc, RapidAI, Annalise, Lunit, Nanox, Gleamer, Blackford, Azra AI, …). Covering someone else's deal hands them the airtime, so such a draft is disqualified **even when it is fact-check CLEAN** (`radiology-care-navigation-software` was, and was still pulled). Stricter than, and separate from, the "no outbound links to rival vendors" rule. Neutral market/policy/regulatory/clinical-evidence/workflow topics are unaffected — a peer-reviewed study is not a vendor announcement.
