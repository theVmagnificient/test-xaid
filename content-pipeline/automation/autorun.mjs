...
4. Competitors — TWO separate rules, both hard:
   (a) No outbound links to rival AI-vendor sites (Viz.ai, Aidoc, RapidAI, Annalise, Lunit, Nanox, Gleamer, Blackford, Azra AI, etc.).
   (b) Do NOT name or promote a competitor or competitor-adjacent vendor's partnership, alliance, funding round or product launch. Founder rule (2026-08-11): an article built around someone else's deal gives them the airtime, so it must not be published even when every fact in it checks out. Verdict is REJECT if the article's SUBJECT is such an announcement; HOLD if it merely name-drops one and removing the mention would not gut the piece. Neutral market, policy, regulatory, clinical-evidence and workflow topics are unaffected — a peer-reviewed study is not a vendor announcement.
